...
<head>
    ...
    <title>Tipo Impresoras</title>
</head>

<body id="body">
    <nav class="navbar navbar-expand-lg navbar-light bg-light" id="navbar">
        <div class="container-fluid">
            <a class="navbar-brand" href="#">Tipo Impresoras</a>
    ...
    <div class="container-fluid d-flex flex-column align-items-center mt-3">
        ...
        <a onclick="addTipo_impresoras()" class="btn btn-outline-primary p-3">Crear un registro nuevo <i class='fas fa-plus' ></i></a>
        ...
        <div class="tabla">
            <table id="equipos" class="table text-center mt-4">
                ...
                <tbody>
                    <?php 
                        if($tipo_impresorasResult ->num_rows>0){
                            while($row = $tipo_impresorasResult ->fetch_assoc()){ 
                    ?>
                    <tr>
                        <td>
                            <a onclick="editTipo_impresoras(<?php echo $row['id']?>, '<?php echo $row['impresora'] ?>')"  class="btn btn-outline-info"><i class="fas fa-edit"></i></a>
                            <a  onclick="deleteTipo_impresoras('<?php echo $row['id']?>', '<?php echo $row['impresora'] ?>')" class="btn btn-outline-danger"><i class="fas fa-trash"></i></a>
                        </td>
                        <td><?php echo $row['id']  ?></td>
                        <td><?php echo $row['impresora'] ?></td>
                    </tr>
                    <?php }} ?>
                </tbody>
            </table>
